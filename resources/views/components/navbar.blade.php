<div class="navbar navbar-custom fixed shadow-md px-4 md:px-20 z-50 transition-all duration-300">
    <div class="navbar-start">
        ...
        {{-- Logo --}}
        <a class="flex items-center font-semibold gap-0 text-xl nav-title px-3 rounded-md"
            href="{{ route('beranda') }}">
        ...
        <ul class="flex items-center gap-2 px-1 text-base font-medium space-x-2">
            <li>
                <a href="{{ route('beranda') }}" class="relative group px-3 py-2">
                    <span>Beranda</span>
                    <span
                        class="absolute bottom-1.5 left-0 w-full h-0.5 bg-[var(--bg-primary)] scale-x-0 group-hover:scale-x-100 transition-transform duration-300 ease-out origin-left"></span>
                </a>
            </li>
            <li>
    ...
    <div class="navbar-end gap-3">
        ...
        @auth
            ...
            <div class="dropdown dropdown-end">
                <div tabindex="0" role="button"
                    ...
                    <div class="w-10 rounded-full ring-2 ring-primary/50 ring-offset-base-100 ring-offset-2">
                        @if (Auth::user()->role === 'mahasiswa' && Auth::user()->mahasiswa)
                            <img src="{{ Auth::user()->mahasiswa->foto_profil ? asset('storage/' . Auth::user()->mahasiswa->foto_profil) : 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->mahasiswa->nama) . '&background=187DAB&color=fff&size=128' }}"
                                alt="Foto Profil Mahasiswa" class="object-cover" />
                        @elseif (Auth::user()->role === 'mitra' && Auth::user()->mitra)
                            <img src="{{ Auth::user()->mitra->logo_perusahaan ? asset('storage/' . Auth::user()->mitra->logo_perusahaan) : 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->mitra->nama_perusahaan) . '&background=187DAB&color=fff&size=128' }}}"
                ...
                <ul tabindex="0"
                    class="menu dropdown-content bg-base-100 rounded-box z-[1] mt-4 w-64 p-2 shadow-xl border border-base-200/50">
                    {{-- Header Dropdown --}}
                    <li class="px-2 pt-2 pb-1">
                        <div class="flex flex-col pointer-events-none">
                            <p class="font-bold truncate">
                                {{ Auth::user()->role == 'mahasiswa' ? Auth::user()->mahasiswa->nama : Auth::user()->mitra->nama_perusahaan }}
                            </p>
